"""
Playwright browser context management for SharePoint MCP.

Uses the system's installed Chrome/Edge with a persistent profile at
~/.sharepoint-mcp-server/browser-profile/ so that:
...
logger = logging.getLogger("sharepoint-mcp.browser")
...
def _get_browser_channel() -> str:
    ...
    try:
        cfg = load_config()
        return cfg.get("sharepoint", {}).get("browser_channel", "chrome")
    except Exception:
        return "chrome"
